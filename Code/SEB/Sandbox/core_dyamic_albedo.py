...
start_date=datetime.datetime(year=2019,month=7,day=11)
...
@jit
def sim_lw(rh,tk,lw,toa,fit_idx):
    ...
    In:
        ...
        - fit_idx (bool) : boolean array indicating which data should be 
                           used for fitting
    ...
    """    
    assert len(rh) == len(tk) == len(lw) == len(toa), "Check input vectors : " + \
    "rh = %.0f, tk = %.0f, lw = %.0f, toa = %.0f" % \
    (len(rh),len(tk),len(lw),len(toa))
    ...
    # Further indices for fitting
    fit_clear_idx=np.logical_and(clear_idx,fit_idx)
    fit_cloudy_idx=np.logical_and(cloudy_idx,fit_idx)
    
    x0=np.array([1.0,1.0,1.0])
    fit_clear=minimize(lw_rdk,x0,args=(rh[fit_clear_idx],tk[fit_clear_idx],\
                                       lw[fit_clear_idx]))
    x_clear=fit_clear.x
    fit_cloudy=minimize(lw_rdk,x0,args=(rh[fit_cloudy_idx],tk[fit_cloudy_idx],\
                                        lw[fit_cloudy_idx]))
...
#_____________________________________________________________________________# 
@jit
def SEB(ta,qa,rho,u,p,sw_i,lw,init_ts,tg_mean,depth,inc,albedo,WF=False):
...
if __name__ == "__main__":
    ...
    if compute:
        ...
        for jj in range(len(fs)):
            ...
            # Perform sw corrections and compute running albedo before we cut
            # out period of interest
            
            # No SW <0
            data["SW_IN_AVG"].loc[data["SW_IN_AVG"]<0]=0
            data["SW_OUT_AVG"].loc[data["SW_OUT_AVG"]<0]=0
            
            # Zero Sw when toa is = 0
            toa,r=sin_toa(data.index.dayofyear.values[:],\
                        data.index.hour.values[:],27.98,86.93)
            data["SW_IN_AVG"].loc[toa==0]=0.0
            data["SW_OUT_AVG"].loc[toa==0]=0.0
            
            # Need albedo to deal with case of SW_OUT > SW_IN
            data["ALBEDO"]=data["SW_OUT_AVG"].rolling(24).sum()/\
            data["SW_IN_AVG"].rolling(24).sum()
            data["ALBEDO"].loc[data["ALBEDO"]>0.90]=0.90
            
            # *** Now we can cut out
            idx=np.logical_and(data.index>=start_date,\
                                         data.index<=end_date)
            data=data.loc[idx]
            toa=toa[idx]
            sw_idx=data["SW_IN_AVG"]<data["SW_OUT_AVG"]
            data["SW_IN_AVG"].loc[sw_idx]=data["SW_OUT_AVG"].loc[sw_idx]/\
            data["ALBEDO"].loc[sw_idx]
            assert np.sum(data["SW_IN_AVG"]<data["SW_OUT_AVG"])==0,"Check SW!"
            
            # Fix temps (issue with the HMP109 -- simply replace with the
            # 109 during this limited period -- for now...)
            ta_err_idx=np.abs(data["T_HMP"]-data["T_109"])>10.
            data["T_HMP"].loc[ta_err_idx]=data["T_109"][ta_err_idx]
            
            # Fix LW during suspected periods of snowcover on pyranometer 
            # Note that we need to only fit during those periods when we 
            # can trust the LW data; then we simulate for all.            
            lw_ref=sim_lw(data["RH"].values[:],data["T_HMP"].values[:]+273.15,\
                          data["LW_IN_AVG"],toa,~sw_idx)
            data["LW_IN_AVG"].loc[sw_idx]=lw_ref[sw_idx]

            # *** NOW resample to higher frequency
            freq="%.0fmin" % (ds/60.)
            data=data.resample(freq).interpolate("linear")
    
            # Assignments for readibility when submitting to functions
            ...
            SEB(ta,qa,rho,u,p,sw,lw,ta[0],tg_mean,depth,inc,albedo,WF=True)
...
## Local params
snow_thresh_lower=0.1
snow_thresh_upper=0.99
melt_thresh=boltz*emiss*273.15**4
dt=datetime.timedelta(days=1)
res="D"


# Test for melting occurrence
test_obs=[input_data["south_col"]]
test_mod=[out[0]]    
for i in range(1):
    
    scratch=test_obs[i].resample(res).mean() # res mean (for albedo)
    scratch_mod=test_mod[i].resample(res).sum()  # res sum of melt
    scratch_obs=(test_obs[i].resample(res).max())["LW_OUT_MAX"] # res max obs LW

    # Create plot
    figx,axx=plt.subplots(1,1)
    figx.set_size_inches(8,4)

    albedo=scratch["ALBEDO"]
    
    # Define indices:
    # 1 -> snow on ground 
    snow_idx=np.logical_and(albedo>=snow_thresh_lower,albedo<=snow_thresh_upper)
    ...
    # Now plot
    # Albedo 
    axx.plot(albedo.index,albedo.values[:],color="k")
