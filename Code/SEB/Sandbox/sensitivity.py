...
def CONDUCT(sub_temps,z,c,cp_i,seb,sw_yes=False,q=None):
    ...
    f[1:] = np.diff(sub_temps)/dz * c # conductance to layer below (len=nT-1)
    f[0] = -seb # -ve down, same convention as conductance
...
if __name__ == "__main__":
    ...
    fins=["/home/lunet/gytm3/Everest2019/AWS/Logging/south_col.csv"]#,\
         #"/home/lunet/gytm3/Everest2019/AWS/Logging/c2.csv" ]
    
    # Create array of z0_m possibilities and albedo possibilities 
    z0_poss=np.linspace(z0_m/10,z0_m*10,50)
    albedo_poss=np.linspace(albedo-0.2,albedo+0.2,50)
    z0_array,albedo_array=np.meshgrid(z0_poss,albedo_poss)
    
    # Preallocate array to store melt/sublimation 
    melt_array=np.zeros(z0_array.shape)
    sub_array=np.zeros(melt_array.shape)
    
    # Dictionary out
#    names=["south_col","c2"]
#    out={names[0]:{},names[1]:{}}
    
    # Iterate over z0_array and albedo array
    for col in range(z0_array.shape[1]):
        
        for row in range(albedo_array.shape[0]):
            
            # Change the global vars z0_m and albedo to be taken from the array
            z0_m=z0_array[row,col]
            albedo = albedo_array[row,col]
            ...
            for fin in fins:
                ...
                # Experiment to see if it melts on Everest's summit!
                #------------------------------
                #ta = ta -(8848-7945)*0.0065
                #-----------------------------
                ...
                # Store mean melt and mean sub in array. Convert to daily by
                # multiplication; more human-readable. 
                melt_array[row,col]=np.nanmean(melt) * scalar
                sub_array[row,col]=np.nanmean(sub) * scalar
                
#                assert row != 3, "Check results so far!"
                # Use Pandas to compute the daily mean
#                scratch=pd.DataFrame(data={"melt":melt,"sub":sub},index=data.index)
#                daily=scratch.resample("D").sum()
                #elapsed=(datetime.datetime.now()-now).total_seconds()
        
        
                # Plot        
        #        fig,ax=plt.subplots(1,1)
        #        out[_n]["melt_daily"].plot(ax=ax)
        #        out[_n]["sub_daily"].plot(ax=ax)
        
                #count+=1
                
        
out={"melt_array":melt_array,"sub_array":sub_array,"z0_array":z0_array,\
     "albedo_array":albedo_array}
pickle.dump(out,open("SEB_sensitivity.p","wb"))
